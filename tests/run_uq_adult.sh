...
for seed in "${SEEDS[@]}"; do
    ...
    python3 tests/uq.py ${COMMON_ARGS} ${SEED_ARGS} --method map --run_name "${BENCHMARK}/map_baseline_seed${seed}"
    ...
    python3 tests/uq.py ${COMMON_ARGS} ${SEED_ARGS} --method laplace --subset_of_weights all --hessian_structure full --approx_type ef --run_name "${BENCHMARK}/laplace_star_baseline_seed${seed}"
    ...
    python3 tests/uq.py ${COMMON_ARGS} ${SEED_ARGS} ${SWAG_LAPLACE_ARGS} --run_name "${BENCHMARK}/swag_laplace_baseline_seed${seed}"
    ...
    for intensity in 0.1 0.25 0.5 0.75 1.0; do
        ...
        python3 tests/uq.py ${COMMON_ARGS} ${SEED_ARGS} --method laplace --subset_of_weights last_layer --noise_intensity ${intensity} --run_name "${BENCHMARK}/noise_${intensity}_seed${seed}"
    done
done
